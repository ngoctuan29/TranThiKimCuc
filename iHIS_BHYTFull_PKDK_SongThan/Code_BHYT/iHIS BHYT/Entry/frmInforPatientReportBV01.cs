...
{
    public partial class frmInforPatientReportBV01 : DevExpress.XtraEditors.XtraForm
    {
        ...
        private string arrPatientReceiveID = string.Empty;
        //private DataTable tbReportBHYT = new DataTable();
        public frmInforPatientReportBV01(string _userid, string _shiftWork, string _patientCode, decimal _patientReceiveID, DateTime _dateInto)
        {
            InitializeComponent();
            this.userid = _userid;
        ...
        private void frmConformReportBV01_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                this.dtSearchFrom.EditValue = this.dtSearchTo.EditValue = Utils.DateServer();
                this.InitField();
                ...
                //this.tbReportBHYT = ReportBHYTBLL.TableListReportBHYTFinish(this.dtSearchFrom.Text, this.dtSearchTo.Text, 0, this.patientCode);
                this.LoadOpenInfoPatient();
            }
            catch (Exception ex)
            {
        ...
        private void GetInfoPatient()
        {
            try
            {
                List<PatientReceive_ViewInf> lstReceive = PatientReceiveBLL.ListForPatient(this.patientCode, this.dateInto.ToString("dd/MM/yyyy"));
                if (lstReceive != null && lstReceive.Count > 0)
                {
                    foreach (var patient in lstReceive)
                    {
                        this.arrPatientReceiveID += patient.PatientReceiveID + ",";
                        if (patient.PatientReceiveID == this.patientReceiveID)
                        {
                            ...
                            this.LoadPatientReceiveAddressCard();
                        }
                    }
        ...
        private void LoadPatientReceiveAddressCard()
        {
            try
            {
                ...
                List<SurviveSignInf> lstSur = SurviveSignBLL.ListSurviveSignForRefID(this.patientReceiveID);
                if (lstSur.Count > 0)
                {
        ...
        private void GetHistoryPatientMedical()
        {
            ...
            List<MedicalRecord_INF> lstMedical = MedicalRecord_BLL.ListMedicalRecordForReceiveID(this.arrPatientReceiveID.TrimEnd(','));
        ...
        public void ListService(ref decimal amountTotalService, bool isReset)
        {
            ...
            if (this.lstViewServies.Count <= 0)
                this.lstViewServies = SuggestedServiceReceiptBLL.ListViewForBV01(this.txtMabn.Text.Trim().ToUpper(), this.arrPatientReceiveID.TrimEnd(','), this.done, this.reportID, this.iPaid).Where(v => v.ObjectCode == 1 && v.DisparityPrice == 0).ToList();
            decimal dBHYT = 0;
            foreach (var v in this.lstViewServies)
            {
                if (v.Check.Equals(1))
        ...
        private void ListMedical(ref decimal amountTotalMedical, bool isReset)
        {
            try
            {
                if (this.lstViewItems.Count <= 0)
                    this.lstViewItems = SuggestedServiceReceiptBLL.ListMedicalViewForBV01(this.txtMabn.Text.Trim().ToUpper(), this.arrPatientReceiveID.TrimEnd(','), this.done, this.reportID).Where(v => v.ObjectCode == 1).ToList();
        ...
        private void butReceipt_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if(!string.IsNullOrEmpty(this.txtTheStart.Text.Trim()))
                    report.StartDate = Utils.StringToDate(this.txtTheStart.Text.Trim());
                if(!string.IsNullOrEmpty(this.txtTheEnd.Text.Trim()))
                ...
                if (!string.IsNullOrEmpty(this.reportID))
                {
                    ...
                    if (result)
                    {
                        PatientReceiveBLL.UpdPatientReceiveConfirmBV01(this.arrPatientReceiveID.TrimEnd(','), 1);
                        ...
                        //this.ClearBHYTInfo();
                        //this.GetPatientWaiting();
                        this.butPrintBV01_Click(sender, e);
                    }
                    else
                    {
        ...
        {
            this.gridControl_BankList.DataSource = ReportBHYTBLL.TableListReportBHYTFinish(this.dtSearchFrom.Text, this.dtSearchTo.Text, 0, this.patientCode);
        }
        ...
        private void butPrintBV01_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (tableDetail != null && tableDetail.Rows.Count > 0)
                {
                    ...
                    rptShow.Parameters["prDisparityPrice"].Value = 0;// Convert.ToDecimal(tableInfo.Rows[0]["TotalSurcharge"].ToString());
        ...
        private void LoadOpenInfoPatient()
        {
            this.ClearBHYTInfo();
            this.GetInfoPatient();
            this.GetTotalServiceMedical(true);
        ...
        private void gridView_BankList_RowStyle(object sender, RowStyleEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    isSendFile = Convert.ToBoolean(view.GetFocusedRowCellValue("SendFile"));
                    if (isSendFile)
        ...
        {
            this.txtPathSaveXML9324.Text = string.Empty;
        }
        ...
        private void butPathSave_Click(object sender, EventArgs e)
        {
            using (var fbd = new FolderBrowserDialog())
            {
                ...
                {
                    this.txtPathSaveXML9324.Text = fbd.SelectedPath;
                }
            }
        }

        private void repbtn_UploadXML_Click(object sender, EventArgs e)
        {
            try
            {
                if (!Convert.ToBoolean(this.gridView_BankList.GetRowCellValue(this.gridView_BankList.FocusedRowHandle, "SendFile")))
                {
                    if (!string.IsNullOrEmpty(this.txtPathSaveXML9324.Text))
                    {
                        string reportIDTemp = this.gridView_BankList.GetRowCellValue(this.gridView_BankList.FocusedRowHandle, "ReportID").ToString();
                        string dateInTemp = this.gridView_BankList.GetRowCellValue(this.gridView_BankList.FocusedRowHandle, "DateInto").ToString();
                        string dateOutTemp = this.gridView_BankList.GetRowCellValue(this.gridView_BankList.FocusedRowHandle, "DateOut").ToString();
                        if (!string.IsNullOrEmpty(reportIDTemp))
                        {
                            string pathSaveXML = string.Empty;
                            #region XML1
                            DataTable tableXML1 = rpt_General_BLL.WriteXML1_9324(dateInTemp.Substring(0, 10), dateOutTemp.Substring(0, 10), reportIDTemp);
                            DataSet dsXML1 = new DataSet("CHITIEUTONGHOPKCBBHYT");
                            tableXML1.TableName = "TONG_HOP";
                            dsXML1.Merge(tableXML1);
                            pathSaveXML = this.txtPathSaveXML9324.Text + "\\XML1.xml";
                            dsXML1.WriteXml(pathSaveXML);
                            #endregion

                            #region XML2
                            DataTable tableXML2 = rpt_General_BLL.WriteXML2_9324(dateInTemp.Substring(0, 10), dateOutTemp.Substring(0, 10), reportIDTemp);
                            DataSet dsXML2 = new DataSet("DSACH_CHI_TIET_THUOC");
                            tableXML2.TableName = "CHI_TIET_THUOC";
                            dsXML2.Merge(tableXML2);
                            pathSaveXML = this.txtPathSaveXML9324.Text + "\\XML2.xml";
                            dsXML2.WriteXml(pathSaveXML);
                            #endregion

                            #region XML3
                            DataTable tableXML3 = rpt_General_BLL.WriteXML3_9324(dateInTemp.Substring(0, 10), dateOutTemp.Substring(0, 10), reportIDTemp);
                            DataSet dsXML3 = new DataSet("DSACH_CHI_TIET_DVKT");
                            tableXML3.TableName = "CHI_TIET_DVKT";
                            dsXML3.Merge(tableXML3);
                            pathSaveXML = this.txtPathSaveXML9324.Text + "\\XML3.xml";
                            dsXML3.WriteXml(pathSaveXML);
                            #endregion

                            Entry.frmUploadHSGDBHYT frmUpload = new Entry.frmUploadHSGDBHYT(this.userid, this.txtPathSaveXML9324.Text);
                            frmUpload.ShowInTaskbar = true;
                            frmUpload.ShowDialog();
                            if (frmUpload.isSendFileResult)
                                ReportBHYTBLL.UpdSendFileReportBHYT(reportIDTemp);
                        }
                        else
                            XtraMessageBox.Show("Chọn bệnh nhân cần upload hồ sơ lên cổng giám định BHXH!", "Bệnh viện điện tử .NET", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                    else
                        XtraMessageBox.Show("Chọn đường dẫn lưu trữ file (XML1, XML2, XML3, XML4, XML5).", "Bệnh viện điện tử .NET", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else
                {
                    XtraMessageBox.Show("Hồ sơ đã được gửi lên cổng giám định! \r\n Không được phép gửi!", "Bệnh viện điện tử .NET", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }
            catch (Exception ex)
