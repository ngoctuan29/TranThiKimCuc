...
{
    public partial class frmSieuAmGeneral : DevExpress.XtraEditors.XtraForm
    {
        ...
        public frmSieuAmGeneral(string _spk, string _uSerid, Int32 _Menu, string _namekp, string _employeeCodeDoctor, string _shiftWork, DateTime _dtWorking)
        {
            InitializeComponent();
        ...
        private void frmSieuAmGeneral_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                this.splitContainerControl1.Panel1.MinSize = 901;
        ...
        private void GetTemplate()
        {
            ...
            {
                DataTable tableTemplate = this.tableTemplateHeader.Select("ServiceCode='" + this.serviceCode + "' and ( ApplyName='Chung' or ApplyName ='"+this.Gender+"')").CopyToDataTable();
                if (tableTemplate != null && tableTemplate.Rows.Count > 0)
                {
        ...
        public void EnableField(bool ena)
        {
            ...
            this.tool_Devices.Enabled = this.tool_Option.Enabled = this.tool_Photo.Enabled = this.tool_SelectImages.Enabled = this.tool_CaptureVideos_Record.Enabled = this.tool_PathSaveFile.Enabled = ena;
            this.tool_CaptureVideos_Stop.Enabled = false;
        }
        ...
        private void butPrint_Click(object sender, EventArgs e)
        {
            try
            {
                if (serviceCode != string.Empty && patientCode != string.Empty)
                {
                    ...
                    {
                        Reports.rptCLS_SieuamA5 rptShow = new Reports.rptCLS_SieuamA5();
                        rptShow.Parameters["RadiologyRowID"].Value = this.receiptCode.ToString();
                        rptShow.Parameters["prDateResult"].Value = this.dtResult;
                        rptShow.DataSource = dsTemp;
                        Reports.frmReportEditGeneral rpt = new Reports.frmReportEditGeneral(rptShow, "SieuAm", "Kết quả siêu âm");
        ...
        private void frmSieuAmGeneral_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                switch (e.KeyCode)
        ...
        private void frmSieuAmGeneral_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (this.capture != null)
        ...
        private void btnExportImage_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            {
                XtraMessageBox.Show("Lỗi khi xuất hình ảnh.\n\n" + ex.Message, "Bệnh viện điện tử .NET", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        private void tool_CaptureVideos_Record_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.capture == null)
                ...
                if (this.patientCode != string.Empty)
                {
                    ...
                    this.tool_CaptureVideos_Record.Enabled = false;
                    this.tool_CaptureVideos_Stop.Enabled = true;
                }
        ...
        private void tool_CaptureVideos_Stop_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.capture == null)
                    throw new ApplicationException("Please select a video and/or audio device.");
                this.capture.Stop();
                this.tool_CaptureVideos_Record.Enabled = true;
                this.tool_CaptureVideos_Stop.Enabled = false;
            }
            catch (Exception ex)
            {
