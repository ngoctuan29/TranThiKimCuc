...
{
    public partial class frmDuoc_TheoDoiDichTruyen : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        private decimal pPatientReceiveID = 0;
        ...
        public frmDuoc_TheoDoiDichTruyen(string _user, string _repositoryCode, string _shiftWork)
        {
            InitializeComponent();
            this.userid = _user;
        ...
        public void LoadList()
        {
            try
            {
                ...
                this.dtMedicalDetail = MedicalRecord_BLL.DTMedicalRecordApproveDV(this.medicalCode, this.iObjectCode, this.repositoryCode, iStatus, this.dRowIDMedicines, this.chkObjectCode.Checked ? 1 : -1);
        ...
        private void frmDuoc_TheoDoiDichTruyen_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void gridView_List_ValidateRow(object sender, DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs e)
        {
            try
            {
                ...
                MedicinesForPatients_ReceiveINF inf = new MedicinesForPatients_ReceiveINF();
                decimal soluongbandau = Convert.ToDecimal(gridView_List.GetRowCellValue(e.RowHandle, "Quantity").ToString());
               

                inf.PatientCode = this.patientCode;
                inf.PatientReceiveID = this.dReceiveID;
                inf.ReferenceCode = gridView_List.GetRowCellValue(e.RowHandle, "MedicalRecordCode").ToString();
                inf.IDate = DateTime.Now;
                inf.EmployeeCode = this.userid;
                inf.ItemCode = gridView_List.GetRowCellValue(e.RowHandle, "ItemCode").ToString();
                inf.RepositoryCode = gridView_List.GetRowCellValue(e.RowHandle, "RepositoryCode").ToString();
                inf.Quantity = Convert.ToDecimal(gridView_List.GetRowCellValue(e.RowHandle, "Quantity").ToString());
                decimal soluong = 0;
                if(gridView_List.GetRowCellValue(e.RowHandle, "Quantity_Receive").ToString()!=null && gridView_List.GetRowCellValue(e.RowHandle, "Quantity_Receive").ToString()!="")
                    soluong = Convert.ToDecimal(gridView_List.GetRowCellValue(e.RowHandle, "Quantity_Receive").ToString());
                if(soluong>soluongbandau)
                {
                    XtraMessageBox.Show(" Số lượng nhận không được lớn hơn SL! ", "Bệnh viện điện tử .NET");
                    return;
                }
                inf.Quantity_Receive = Convert.ToDecimal(soluong);
                inf.WorkDate_Receive = DateTime.Now;
                inf.Note = gridView_List.GetRowCellValue(e.RowHandle, "Note").ToString();
                inf.RowIDMedicine = Convert.ToDecimal(this.dRowIDMedicines);

                    if (MedicinesForPatientsBLL.MedicinesForPatients_ReceiveIns(inf) == 1)
                    {
