...
namespace ClinicBLL
{
    public class BanksAccountBLL
    {
        ...
        public static DataTable DT_StatisticBankTotal(string fromDate, string toDate, string serviceGroupCode, string serviceCategoryCode, string serviceCode)
        {
            ...
            tableResult.Columns.Add("PostingTime",typeof(DateTime));
            ...
            tableResult.Columns.Add("TotalPHUTHU", typeof(decimal)).DefaultValue = 0;
            tableResult.Columns.Add("TotalDisparity", typeof(decimal)).DefaultValue = 0;
            try
            {
                ...
                if (tableTemp != null && tableTemp.Rows.Count > 0)
                {
                    foreach (DataRow row in tableTemp.Rows)
                    {
                        ...
                        decimal TotalDisparity = Convert.ToDecimal(row["DisparityPrice"]);
                        if (r == null)
                        {
                            ...
                            addRow["TotalDisparity"] = TotalDisparity;
                            tableResult.Rows.Add(addRow);
                        }
                        else
                        {
                            if (row["ServiceModuleCode"].ToString() == "KCB")
                                r["TotalMedical"] = Math.Round(Convert.ToDecimal(r["TotalMedical"].ToString()),2) + Math.Round(Convert.ToDecimal(row["Amount"].ToString()),2);
                            if (row["ServiceModuleCode"].ToString() == "THUOC")
                                r["TotalDrug"] = Math.Round(Convert.ToDecimal(r["TotalDrug"].ToString()),2) + Math.Round(Convert.ToDecimal(row["Amount"].ToString()),2);
                            if (row["ServiceModuleCode"].ToString() == "CDHA")
                                r["TotalRad"] = Math.Round(Convert.ToDecimal(r["TotalRad"].ToString()),2) + Math.Round(Convert.ToDecimal(row["Amount"].ToString()),2);
                            if (row["ServiceModuleCode"].ToString() == "XN")
                                r["TotalLab"] = Math.Round(Convert.ToDecimal(r["TotalLab"].ToString()),2) + Math.Round(Convert.ToDecimal(row["Amount"].ToString()),2);
                            if (row["ServiceModuleCode"].ToString() == "PTTT")
                                r["TotalPTTT"] = Math.Round(Convert.ToDecimal(r["TotalPTTT"].ToString()),2) + Math.Round(Convert.ToDecimal(row["Amount"].ToString()),2);
                            if (row["ServiceModuleCode"].ToString() == "TC")
                                r["TotalTC"] = Math.Round(Convert.ToDecimal(r["TotalTC"].ToString()),2) + Math.Round(Convert.ToDecimal(row["Amount"].ToString()),2);
                            if (row["ServiceModuleCode"].ToString() == "VC")
                                r["TotalVC"] = Math.Round(Convert.ToDecimal(r["TotalVC"].ToString()),2) + Math.Round(Convert.ToDecimal(row["Amount"].ToString()),2);
                            if (row["ServiceModuleCode"].ToString() == "GIUONG")
                                r["TotalGIUONG"] = Math.Round(Convert.ToDecimal(r["TotalGIUONG"].ToString()),2) + Math.Round(Convert.ToDecimal(row["Amount"].ToString()),2);
                            if (row["ServiceModuleCode"].ToString() == "NHAKHOA")
                                r["TotalNHAKHOA"] = Math.Round(Convert.ToDecimal(r["TotalNHAKHOA"].ToString()),2) + Math.Round(Convert.ToDecimal(row["Amount"].ToString()),2);
                            r["TotalDisparity"] =  Math.Round(Convert.ToDecimal(r["TotalDisparity"].ToString()),2) + TotalDisparity;
                            r["TotalAll"] = Math.Round(Convert.ToDecimal(r["TotalAll"].ToString()),2) + Math.Round(Convert.ToDecimal(row["Amount"].ToString()),2) + TotalDisparity;
                        }
                    }
                }
                //thuốc bán lẻ
